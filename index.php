...
<body class="bg-[#4d36b2]">
    ...
    <div class="header finisher-header container min-h-screen" id="Hero">
        <div class="grid">
            <h1 class="text-[72px] md:text-[50px] font-semibold lg:mt-20 mt-10 text-white text-center font-DmSans">OLERACARE</h1>
...
<script>
    ...
    function detectColor() {
        // Ambil warna dari tengah kamera
        ...
        // Konversi RGB ke HEX
        const detectedHex = rgbToHex(pixel[0], pixel[1], pixel[2]);
        console.log("Warna terdeteksi:", detectedHex);

        // Cari warna pH yang paling mendekati
        const closestPh = findClosestPh(detectedHex);
        phValue.innerText = closestPh.ph;
        ...
        fetch(`backend/get_ph_info.php?ph=${closestPh.ph}`)
    ...
    function findClosestPh(hexColor) {
        let detectedRgb = hexToRgb(hexColor);
        let closest = phDataset[0];
        let minDistance = Number.MAX_VALUE;

        phDataset.forEach(data => {
            let datasetRgb = hexToRgb(data.hex);
            let distance = colorDistance(detectedRgb, datasetRgb);
